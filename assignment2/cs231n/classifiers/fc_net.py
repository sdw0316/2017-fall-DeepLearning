...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ############################################################################
        self.params["W1"] = weight_scale * np.random.randn(input_dim, hidden_dim) # (N, H)
    ...
    def loss(self, X, y=None):
        ...
        H, hidden_cache = affine_relu_forward(X, W1, b1)
        scores, out_cache = affine_forward(H, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # softmax loss
        data_loss, dy = softmax_loss(scores, y)
        reg_loss = (0.5 * reg * np.sum(W1*W1)) + (0.5 * reg * np.sum(W2*W2))
        loss = data_loss + reg_loss
        # backprop
        dh, grads["W2"], grads["b2"] = affine_backward(dy, out_cache)
        dx, grads["W1"], grads["b1"] = affine_relu_backward(dh, hidden_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        ############################################################################
        # (1) params for input-to-hidden 
        self.params['W1'] = np.random.randn(input_dim, hidden_dims[0]) * weight_scale
        self.params['b1'] = np.zeros([hidden_dims[0]]) 
        
        # (2) params for hidden-to-hidden
        for i in range(self.num_layers - 2):
          self.params['W%d' %(i+2)] = np.random.randn(hidden_dims[i], hidden_dims[i+1]) * weight_scale
          self.params['b%d' %(i+2)] = np.zeros([hidden_dims[i+1]])
        
        # (3) params for hidden-to-out
        self.params['W%d' %(self.num_layers)] = np.random.randn(hidden_dims[i+1], num_classes) * weight_scale
